...
export function generateBalancedTeams(players: Player[], tierPoints: TierPoints): CustomMatch {
  ...
  const totalValue = players.reduce((sum, player) => sum + calculatePlayerValue(player, tierPoints), 0);
  ...
  const blueTeam: Team = {
    ...
    averageRank: isTeamABlue ? calculateTeamAverage(teamA, tierPoints) : calculateTeamAverage(teamB, tierPoints),
  };
  
  const redTeam: Team = {
    players: isTeamABlue ? teamB : teamA,
    averageRank: isTeamABlue ? calculateTeamAverage(teamB, tierPoints) : calculateTeamAverage(teamA, tierPoints),
  };
...
export function generateTeamName(): string {
  const adjectives = [
    'Mighty', 'Fearless', 'Savage', 'Divine', 'Elite', 
    'Blazing', 'Thunder', 'Mystic', 'Shadow', 'Infernal',
  ];
  
  const nouns = [
    'Dragons', 'Wolves', 'Titans', 'Legends', 'Guardians',
    'Phoenix', 'Knights', 'Demons', 'Warriors', 'Assassins',
  ];
