...
export const CustomsForm: React.FC<CustomsFormProps> = ({ onTeamsGenerated }) => {
  const [players, setPlayers] = useState<Player[]>(
    Array(10).fill(null).map(() => ({ name: '', rank: '' }))
  );
  ...
  const fillTestData = () => {
    const testNames = [
      'ShadowBlade', 'MoonStriker', 'FrostFire', 'ThunderGod', 'NightWalker',
      'BloodRaven', 'StormRider', 'DragonFist', 'PhantomSlayer', 'WildHeart'
    ];
  ...
  const resetForm = () => {
    setPlayers(Array(10).fill(null).map(() => ({ name: '', rank: '' })));
    setError(null);
  };
  
  return (
    <div className="bg-dark-100 rounded-lg p-6 shadow-lg">
      <h2 className="text-2xl font-bold mb-6 text-center border-b border-dark-300 pb-3">Custom 5v5 Team Generator</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-4 mb-8">
        <div className="bg-dark-200/70 p-4 rounded-md shadow-md">
          <h3 className="text-lg font-medium text-gray-300 mb-3 border-b border-dark-300 pb-2">Team 1</h3>
          <div className="space-y-3">
            {players.slice(0, 5).map((player, index) => (
              <div key={index} className="bg-dark-200 p-3 rounded-md border border-dark-300 hover:border-gray-500 transition-colors">
        ...
        <div className="bg-dark-200/70 p-4 rounded-md shadow-md">
          <h3 className="text-lg font-medium text-gray-300 mb-3 border-b border-dark-300 pb-2">Team 2</h3>
          <div className="space-y-3">
            {players.slice(5, 10).map((player, index) => (
              <div key={index + 5} className="bg-dark-200 p-3 rounded-md border border-dark-300 hover:border-gray-500 transition-colors">
                <div className="flex items-center justify-between mb-2">
                  <div className="text-sm font-medium text-gray-400">
                    Player {index + 6}
                  </div>
